...
function getScreenConfig() {
  ...
  return {
    burstCount: Math.round(46 * Math.max(0.7, areaRatio)),
...
function emitBurst(x, y, angleMin, angleMax, config) {
  const count = config.burstCount + Math.floor(Math.random() * 12);
...
/**
 * 一次性瞬间喷发，根据屏幕尺寸自动调整密度
 */
function instantBurst() {
  const config = getScreenConfig();
  const burstsPerWave = Math.max(6, Math.round(10 * config.areaRatio));
  ...
  setTimeout(() => {
    for (let i = 0; i < Math.ceil(burstsPerWave * 0.35); i++) {
      emitFromLeft(config);
      emitFromRight(config);
    }
  }, 60);
  ...
  setTimeout(() => {
    for (let i = 0; i < Math.ceil(burstsPerWave * 0.2); i++) {
      emitFromLeft(config);
      emitFromRight(config);
    }
  }, 110);
  ...
  setTimeout(() => {
    for (let i = 0; i < Math.ceil(burstsPerWave * 0.15); i++) {
      emitFromLeft(config);
      emitFromRight(config);
    }
  }, 180);
  ...
  setTimeout(() => {
    for (let i = 0; i < Math.ceil(burstsPerWave * 0.1); i++) {
      emitFromLeft(config);
      emitFromRight(config);
    }
  }, 250);
  ...
  setTimeout(() => {
    for (let i = 0; i < Math.ceil(burstsPerWave * 0.08); i++) {
      emitFromLeft(config);
      emitFromRight(config);
    }
  }, 310);
