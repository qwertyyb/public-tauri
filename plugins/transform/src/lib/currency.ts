...
const fetchExchangeRate = async () => {
  ...
  const json: { currency: string; para1: string; middlerate: number; para2: string }[] =    await r.json();
  const USDtoCNY = json.find(i => i.currency === '美元' && i.para1 === '人民币')!;
  const EUtoCNY = json.find(i => i.currency === '欧元' && i.para1 === '人民币')!;
  const JPtoCNY = json.find(i => i.currency === '日元' && i.para1 === '人民币')!;
  const HKtoCNY = json.find(i => i.currency === '港元' && i.para1 === '人民币')!;
  return {
