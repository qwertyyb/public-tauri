...
<template>
  <label class="rv-form-field">
    ...
    <input
      ...
      :value="textValue()"
      @input="callChange(($event.target as HTMLInputElement).value)"
      @focus="emitFormEvent('focus', ($event.target as HTMLInputElement).value)"
      @blur="emitFormEvent('blur', ($event.target as HTMLInputElement).value)"
    >
    <small
      v-if="info || error"
      class="rv-form-help"
    ...
    <RaycastDetailMarkdown
      v-if="enableMarkdown && textValue().trim()"
