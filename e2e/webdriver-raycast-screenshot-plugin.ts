/**
 * E2E: converted Raycast screenshot plugin.
 *
 * Prerequisites:
 * 1. `pnpm tauri:dev` is running with WebDriver enabled.
 * 2. The Raycast screenshot source exists at `RAYCAST_SCREENSHOT_SOURCE_PATH`
 *    (default: ~/Downloads/raycast-screenshot).
 *
 * Run:
 *   pnpm test:webdriver:raycast-screenshot
 */
...
const DEFAULT_SOURCE_DIR = path.join(process.env.HOME || '', 'Downloads/raycast-screenshot');
const SOURCE_DIR = process.env.RAYCAST_SCREENSHOT_SOURCE_PATH
  ? path.resolve(process.env.RAYCAST_SCREENSHOT_SOURCE_PATH)
  : DEFAULT_SOURCE_DIR;
const PLUGIN_DIR = process.env.RAYCAST_SCREENSHOT_PLUGIN_PATH ?? '/tmp/public-tauri-raycast-screenshot';
...
function ensureScreenshotPluginConverted(): void {
  if (process.env.E2E_SKIP_PLUGIN_BUILD === '1') {
    console.log('[e2e] Skip Raycast screenshot conversion (E2E_SKIP_PLUGIN_BUILD=1)');
    return;
  }

  console.log('[e2e] Converting Raycast screenshot plugin...');
...
async function ensureHookAndRegisterPlugin(driver: WebDriver, pluginDir: string, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`注册 screenshot 插件超时: ${String(lastErr)}`);
}
...
async function waitForVisibleResultIcon(driver: WebDriver): Promise<void> {
  ...
  throw new Error('Capture Screen result icon not visible');
}

async function main(): Promise<void> {
  ensureScreenshotPluginConverted();
  ...
  try {
    ...
    await setMainInputValue(driver, 'Capture Screen');
    await driver.sleep(1_200);

    const searchText = String(await driver.executeScript('return document.body.innerText || "";'));
    console.log('[e2e] body after search:');
    console.log(searchText.slice(0, 800));
    if (!searchText.includes('Capture Screen')) {
      throw new Error('Capture Screen result not visible');
    }
    await waitForVisibleResultIcon(driver);

    await pressEnterViaActions(driver);
    await driver.sleep(2_000);
    console.log('[e2e] Capture Screen command triggered');
  } finally {
    await driver.quit();
  }
}
