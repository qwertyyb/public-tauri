...
const utils = {
  ...
  'system.runAppleScript': (script: string) => {
    return runAppleScript(script)
  },
  'system.runCommand': (command: string) => {
  ...
  'storage.allItems': (keyPrefix) => {
    return storage.allItems(keyPrefix)
  },
  'storage.removeItem': (key: string) => {
    return storage.removeItem(key)
  },
  'storage.clear': (keyPrefix: string) => {
    return storage.clear(keyPrefix)
  }
}
